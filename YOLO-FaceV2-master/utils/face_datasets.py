...
class LoadFaceImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        if self.rect:
            # Sort by aspect ratio            s = self.shapes  # wh
            s = self.shapes
        ...
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                ...
                try:
                    ...
                    im.verify()  # PIL验证图像完整性
                    shape = exif_size(im)  # 图像尺寸 (w, h)
                    assert (shape[0] > 9) & (shape[1] > 9), f'图像尺寸过小: {shape} (需>10像素)'
                except Exception as e:
                    ...
                    try:
                        with open(lb_file, 'r') as f:
                            ...
                            for line_idx, line in enumerate(lines):
                                ...
                                if len(parts) != 15:
                                    raise ValueError(f"第{line_idx + 1}行格式错误: 需15列，实际{len(parts)}列")
                            # 转换为numpy数组
                            l = np.array([line.split() for line in lines], dtype=np.float32)
                        # 验证标签内容
                        assert l.shape[1] == 15, f"标签列数错误: 需15列，实际{l.shape[1]}列"
                        assert (l >= -1).all(), f"存在负标签 (允许的最小值为-1): {l[l < -1]}"
                        assert (l[:, 1:] <= 1).all(), f"坐标超出[0,1]范围: {l[:, 1:][l[:, 1:] > 1]}"
                        assert np.unique(l, axis=0).shape[0] == l.shape[0], "存在重复标签"
                    except Exception as e:
                        raise ValueError(f"标签文件解析失败: {e}")
                else:
                    nm += 1  # 标签缺失
                ...
                nc += 1  # 标记为损坏
                # 输出详细错误信息，定位问题文件和原因
                print(f"\n警告: 图像或标签损坏 - 图像文件: {im_file}, 标签文件: {lb_file}")
                print(f"错误原因: {str(e)}")
